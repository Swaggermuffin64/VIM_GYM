...
export class RoomManager {
  ...
  private scheduleWaitingRoomTimeout(roomId: string, isPublic: boolean): void {
    ...
    const timer = setTimeout(() => {
      ...
      if (room && room.state === 'waiting') {
        console.log(
          `⏰ Room ${roomId} waiting timeout - no race started, cleaning up`
        );
    ...
    console.log(
      `⏱️ Scheduled waiting timeout for room ${roomId} in ${timeoutMs / 1000}s (${isPublic ? 'public' : 'private'})`
    );
  }
  ...
  handleTaskComplete(
    ...
    if (
      ...
      Date.now() - player.taskStartedAt < this.MIN_TASK_COMPLETION_MS
    ) {
      return;
    }
  ...
  private async endRace(roomId: string): Promise<void> {
    const room = this.rooms.get(roomId);
    if (!room) return;
    ...
    const enrichedRankings = rankings.map((r) => ({
      ...r,
      ranks: ranksMap.get(r.playerId) ?? null,
    }));
    this.io.to(roomId).emit('game:complete', { rankings: enrichedRankings });
  ...
  private scheduleRoomCleanup(roomId: string): void {
    ...
    console.log(
      `⏱️ Scheduled cleanup for room ${roomId} in ${this.ROOM_IDLE_TIMEOUT_MS / 1000}s`
    );
  }
  ...
  private destroyRoom(roomId: string, reason?: string): void {
    ...
    console.log(
      `🗑️ Destroying room ${roomId} (players: ${room.players.size}, state: ${room.state}, public: ${room.isPublic})`
    );
    ...
    if (room.players.size > 0) {
      const message =
        reason ||
        (room.isPublic ? 'Match ended' : 'Room closed due to inactivity');
    ...
    console.log(
      `✅ Room ${roomId} destroyed. Remaining rooms: ${this.rooms.size}`
    );
  }
  ...
  findOrCreateQuickMatchRoom(
    ...
    for (const [roomId, room] of this.rooms) {
      if (
        ...
        if (joinedRoom) {
          console.log(
            `🎯 Quick match: ${playerName} joined existing room ${roomId}`
          );
