...
function shouldPreferCandidateOnTie(
  ...
  return (
    getMotionPreferenceScore(candidateSequence) >
    getMotionPreferenceScore(currentSequence)
  );
}

function getMotionKeysForOffset(
  offset: number,
...
class minHeap {
  ...
  private bubbleUp(index: number): void {
    ...
    while (curr > 0) {
      ...
      if (!currItem || !parentItem || parentItem.distance <= currItem.distance)
        break;
  ...
  private bubbleDown(index: number): void {
    ...
    while (true) {
      ...
      if (
        ...
        smallestItem &&
        leftItem &&
        leftItem.distance < smallestItem.distance
      ) {
        smallest = left;
      }
      ...
      if (
        ...
        nextSmallestItem &&
        rightItem &&
        rightItem.distance < nextSmallestItem.distance
      ) {
        smallest = right;
      }
...
function getLazyNeighbors(
  ...
  for (const key of vimKeys) {
    ...
    const keyMaxFactor = findMaxFactor(
      state.offset,
      key,
      codeSnippet,
      state.preferredX
...
export function shortestVimSequenceLazy(
  ...
  const startLineRange =
    startLine >= 0 ? codeSnippet.lineOffsetRanges[startLine] : undefined;
  const defaultPreferredX = startLineRange
    ? startingOffset - startLineRange[0]
    : 0;
  ...
  while (!heap.isEmpty()) {
    ...
    for (const neighbor of neighbors) {
      ...
      const candidateSequence = [
        ...currentBest.sequence,
        ...neighbor.keySequence,
      ];
...
function shortestVimSequenceLazyToAnyTarget(
  ...
  const uniqueTargets = new Set<number>(
    targetOffsets.filter(
      (offset) => offset >= 0 && offset < codeSnippet.code.length
    )
  );
...
function setCachedShortestPathResult(
  key: string,
  value: shortestPathCacheEntry
): void {
...
function replaySequenceEndState(
  ...
  for (const token of sequence) {
    ...
    for (let i = 0; i < repeatCount; i++) {
      [offset, preferredX] = resolveKeyOffset(
        offset,
        token,
        codeSnippet,
        preferredX
...
function buildVisualDeletePlan(
  ...
  const [secondNavWeight, secondNavSequence] =
...
function getDeleteRangeForCountedE(
  ...
  const startLineRange =
    startLineNumber >= 0
      ? codeSnippet.lineOffsetRanges[startLineNumber]
      : undefined;
...
function getInnerPairsForStrategy(
  codeSnippet: codeSnippet,
  strategy: DeleteStrategy
): codeSnippet['curlyBraceIndices'] {
...
function getValidInnerTextObjectAnchors(
  ...
  const targetExists = pairs.some(([openOffset, closeOffsetExclusive]) => {
    return (
      openOffset === targetOpen && closeOffsetExclusive === targetCloseExclusive
    );
  });
  const len = codeSnippet.code.length;
  const base = [from, Math.max(from, to - 1), from - 1, to];
  if (!targetExists) {
    return uniqueValidOffsets(base, len);
  }

  // Handful of anchors for multi-target Dijkstra (not a full O(|code|×|pairs|) scan).
  // Covers on/near delimiters and mid-inner; `shortestVimSequenceLazyToAnyTarget` picks the cheapest.
  // Tradeoff: rare valid cursors (e.g. far left where "next pair" still hits this target) are omitted,
  // so the recommended nav may be longer than with a full enumeration.
  ...
  // First column of the line(s) holding the inner span or `{` — often enough for `di{` alone
  // (e.g. `$`-style nav loses to `^`/`0`-style when column 0 is a valid text-object anchor).
  for (const ref of [from, from - 1]) {
    if (ref < 0 || ref >= len) continue;
    const line = getLineFromOffset(ref, codeSnippet);
    if (line < 0) continue;
    const lineRange = codeSnippet.lineOffsetRanges[line];
    if (lineRange) {
      candidates.push(lineRange[0]);
    }
  }
  return uniqueValidOffsets(candidates, len);
}
...
export function getRecommendedDeleteSequence(
  ...
  if (strategy === 'WORD') {
    ...
    const matchingCounts = getMatchingCountedEMotions(codeSnippet, from, to);
  ...
  if (
    strategy === 'INNER_CURLY_BRACE' ||
    strategy === 'INNER_PARENTHESIS' ||
    strategy === 'INNER_BRACKET'
  ) {
    const textObjectChar =
      strategy === 'INNER_CURLY_BRACE'
        ? '{'
        : strategy === 'INNER_PARENTHESIS'
          ? '('
          : '[';
    const anchors = getValidInnerTextObjectAnchors(
      codeSnippet,
      strategy,
      from,
      to
