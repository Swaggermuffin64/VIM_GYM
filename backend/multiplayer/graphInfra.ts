...
export function resolveKeyOffset(
  offset: number,
  key: string,
  codeSnippet: codeSnippet,
  savedRelativeX: number
): IntTuple {
  const lineOffsetRanges = codeSnippet.lineOffsetRanges;
  if (!lineOffsetRanges) return [offset, savedRelativeX];
  const lineStartOffsets =
    codeSnippet.precomputed?.lineStartOffsets ??
    lineOffsetRanges.map((range) => range[0]);
  ...
  switch (motion) {
    ...
    case 'j': {
      ...
      if (nextLineLength <= savedRelativeX) {
        //including newline so <=
    ...
    case 'w': {
      ...
      if (!nextWordIndices)
        return [
          codeSnippet.code.length - 1,
          codeSnippet.code.length - 1 - currentLineStart,
        ];
      const newOffset = nextWordIndices[0];
      const lineStart =
        lineStartOffsets[getLineFromOffset(nextWordIndices[0], codeSnippet)];
    ...
    case 'e': {
      ...
      if (
        (offset >= currentWordEnd || offset >= currentWordEndExclusive) &&
        nextWord
      ) {
        targetWord = nextWord;
      }
    ...
    case 'b': {
      ...
      const newOffset =
        offset === currWordStart && previousWordRange
          ? previousWordRange[0]
          : currWordStart;
      const newLineNumber = getLineFromOffset(newOffset, codeSnippet);
      const newLineStart = lineStartOffsets[newLineNumber];
    ...
    case 'f': {
      ...
      if (foundOffset === -1 || foundOffset > lineEnd)
        return [offset, savedRelativeX];
...
export function multiKeyResolve(
  initialOffset: number,
  key: string,
  factor: number,
  codeSnippet: codeSnippet,
  startingRelativeX: number
): IntTuple[] {
...
export function findMaxFactor(
  ...
  while (i < codeSnippet.code.length + 1) {
    const [currOffset, currRelativeX] = resolveKeyOffset(
      prevOffset,
      key,
      codeSnippet,
      prevRelativeX
...
export function findWordIndex(
  ...
  for (const [i, [wordStart, afterWordEnd]] of wordRanges.entries()) {
    ...
    const nextWordStart = nextWordRange
      ? nextWordRange[0]
